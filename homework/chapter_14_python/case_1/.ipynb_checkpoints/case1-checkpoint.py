...
# ps , ох намучился я с этой задачей , причем буквально когда понял что у меня не получается подогнать ответ (я был близко) начал делать с друзьями которые скажу так что дали аналогичные решения моим , друзья же работают непосредственно аналитиками (сбер и автокомпания)
